...
for (let i = 0; i < cats.length; i++) {
    function createDiv(){
        let bo = document.getElementById("idi")
        let divCard = document.createElement('div')
    
        bo.appendChild(divCard)
        divCard.className="card"
        divCard.id="card"+i
    }
    
    function addH2(){
        let bo = document.getElementById("card"+i)
        let h1 = document.createElement("h1")
        h1.innerHTML=`${cats[i].name}`
        h1.className="title"
    ...
    function nutrDi(){
        let bo = document.getElementById("di"+i)
        let p1 = document.createElement("h3")
        let p2 = document.createElement("h3")
        let p3 = document.createElement("h3")
        p1.textContent=`Возраст: ${cats[i].age}`
        p3.textContent=`Оценка: ${cats[i].rate}`
        if(cats[i].favourite == true){
            p2.textContent=`Фаворит: Да`
        } else{
            p2.textContent=`Фаворит: Нет`
        }
    ...
    function nutrDi2(){
        let bo = document.getElementById("di2"+i)
        let dis = document.createElement("h2")
        let disp = document.createElement("p")
        dis.innerHTML = "Описание:"
        disp.innerHTML = cats[i].description
